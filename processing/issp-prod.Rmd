...
pacman::p_load(dplyr, tidyr, haven, stargazer, sjmisc, sjPlot, sjlabelled, tidyverse, summarytools, ggplot2, ggpubr, texreg, webshot, survey, srvyr, ggrepel, kableExtra)
```
...
box_perc <- ggplot(boxplot_perc, aes(x=oficio, y=value, fill=year)) + 
  geom_boxplot() +
  scale_y_continuous(name = "Ingreso", limits = c(0, 6000000))+
...
box_just <- ggplot(boxplot_just, aes(x=oficio, y=value, fill=year)) + 
  geom_boxplot() +
  scale_y_continuous(name = "Ingreso", limits = c(0, 6000000))+
  scale_x_discrete(name= "Oficio",
                   label = c("Doctor",
...
box_brecha <- ggplot(boxplot_brecha, aes(x=brecha, y=value, fill=year)) + 
  geom_boxplot() +
