...
reg5.1 <- rq(brecha_just ~ sex, data=dat)
reg6.1 <- rq(brecha_just ~ sex + educacion_rec, data=dat)
reg7.1 <- rq(brecha_just ~ sex*educacion_rec, data=dat)
reg8.1 <- rq(brecha_just ~ sex*educacion_rec + year, data=dat)
reg9.1 <- rq(brecha_just ~ sex*educacion_rec + year + brecha_perc, data=dat)

screenreg(list(reg1.1, reg2.1, reg3.1,reg4.1, reg5.1, reg6.1, reg7.1, reg8.1, reg9.1))
```
...
```{r}
htmlreg(list(reg1.1, reg2.1, reg3.1,reg4.1, reg5.1, reg6.1, reg7.1, reg8.1, reg9.1),
        custom.header = list("Brecha percibida"=1:4, "Brecha justa"=5:9),
        custom.model.names = c("Modelo 1",
                               "Modelo 2",
                               "Modelo 3",
                               "Modelo 4",
        ...
        caption = "",
        caption.above = TRUE,
        custom.gof.rows=list("R2" = c(R1.1,R2.1,R3.1,R4.1,R5.1,R6.1,R7.1,R8.1,R9.1)),
        file = "output/tables/reg_brechas_interac.html")

webshot::webshot(url ="output/tables/reg_brechas_interac.html", file ="output/tables/reg_brechas_interac.png")
```