...
salario_real_long$oficio <- factor(salario_real_long$oficio, levels = c("propio", "cajero_hombre", "cajero_mujer", "doctor_hombre", "doctor_mujer", "gerente_hombre", "gerente_mujer", "diputado_hombre", "diputado_mujer"))
```
...
p12_sal_perc_h <- salario_real_long %>% filter(sexo=="Hombre") %>% 
  ...
  scale_x_discrete(label = c("Salario propio", "Cajero Hombre", "Cajera Mujer", "Doctor Hombre", "Doctora Mujer", "Gerente Hombre", "Gerente Mujer", "Diputado Hombre", "Diputada Mujer"))
...
salario_ideal_long$oficio <- factor(salario_ideal_long$oficio, levels = c("propio", "cajero_hombre", "cajero_mujer", "doctor_hombre", "doctor_mujer", "gerente_hombre", "gerente_mujer", "diputado_hombre", "diputado_mujer"))
```
...
base_long$tipo <- factor(base_long$tipo, labels = c("Salario ideal", "Salario percibido"))
...
* salario ideal
```{r}
plot_ideal_educ_sexo <- base_long %>% filter(tipo=="Salario ideal") %>% 
  ggplot(aes(x = oficio, y = mean, color=sexo)) + 
  geom_point(size=4, aes(shape=sexo)) +
  labs(color = "sexo", shape="sexo") +
  xlab("Oficio") + ylab("Salario")  +
